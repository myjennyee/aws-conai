...
import ollama
import pandas as pd
from API.WebServices import call_aspx_webmethod, call_llama_api, get_chat_history, call_llama_api_system, save_chat_message
...
if "chat_history" not in st.session_state:
    ...
    if isinstance(st.session_state.chat_history, list):
        ...
        for i, msg in enumerate(history):
            if isinstance(msg, dict):  # If already in correct format, use as-is
                formatted_history.append(msg)
            elif isinstance(msg, str):  # If raw string, assign role based on position
...
if user_input:
    ...
    save_chat_message( userId= "3", role= "user", message= user_input)

    # Determine response source
    if search_system_data:
        bot_response = call_llama_api_system(user_input)
    else:
        #response = ollama.chat(model="llama3", messages=[{"role": "user", "content": user_input}])
        bot_response = call_llama_api(user_input)# response.get("message", {}).get("content", "Error: No response received")

    st.session_state.chat_history.append({"role": "assistant", "content": bot_response})
    save_chat_message( userId= "3", role= "assistant", message= bot_response)
    ...
    if search_system_data:
        ...
        save_chat_message( userId= "3", role= "assistant", message= bot_response_result)
